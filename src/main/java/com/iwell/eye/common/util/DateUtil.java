...
public class DateUtil {
	...
	//HH:ss
	public static final String TIME_FORMAT_TIME_DASH = "HH:mm";

	// 기본 날짜 포맷 길이
	public static final int DEFAUT_DATE_FORMAT_LEN = DEFAULT_DATE_FORMAT.length();

	// 기본 날짜 포맷 길이
	public static final int DEFAUT_DATE_TIME_FORMAT_LEN = DATE_TIME_FORMAT.length();

	public static final int DEFAULT_DATE_TIME_YYYYMMDDHHMMSS = YYYYMMDDHHMMSS.length();
	...
	public static boolean isVaildDateTimeFormat(String dateStr, boolean nullOk) {
		...
		try {
			...
			if (strLen == DEFAUT_DATE_TIME_FORMAT_LEN) {
				LocalDateTime.parse(dateStr, DateTimeFormatter.ofPattern(DATE_TIME_FORMAT));
			} else {
				LocalDateTime.parse(dateStr, DateTimeFormatter.ofPattern(YYYYMMDDHHMM));
			}
	...
	public static boolean isValidDateYYYYMMDDHHMMSS(String dateStr, boolean nullOk){
		if (nullOk == true && dateStr == null) {
			return true;
		}
		...
		try {
			int strLen = dateStr.length();
			if (strLen == DEFAULT_DATE_TIME_YYYYMMDDHHMMSS) {
	...
	public static boolean isVaildDateTimeSecondFormat(String dateStr, boolean nullOk) {
		if (nullOk == true && dateStr == null) {
			return true;
		}
	...
	//20-08시 사이인지 체크
	public static boolean isDenyTime() throws ParseException {
		SimpleDateFormat format = new SimpleDateFormat(TIME_FORMAT_TIME_DASH);
		Date start = format.parse("20:00");
		Date end = format.parse("08:00");
		LocalDateTime dateTime = LocalDateTime.now();
		dateTime.format(DateTimeFormatter.ofPattern(TIME_FORMAT_TIME_DASH));
		Date now =format.parse(dateTime.format(DateTimeFormatter.ofPattern(TIME_FORMAT_TIME_DASH)));
	...
	//기준일 개월 초과 여부
	//오늘날짜보다 이후면 true
	public static boolean isOverDate(String standardDt, int overDue){

		try {

			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			Date date = new Date(sdf.parse(standardDt).getTime());
			Calendar cal = Calendar.getInstance();
			cal.setTime(date);
			cal.add(Calendar.MONTH, overDue);
			Date OverDt = cal.getTime();
			Date toDay = new Date();

			if(toDay.compareTo(OverDt) > 0){
				return true;
			}
	...
	public static String rsvFormatter(String dateStr){
		int strLen = dateStr.length();
		if (strLen != DEFAUT_DATE_TIME_FORMAT_LEN) {
			dateStr = getFormatDateTime(dateStr+"00");
		}
	...
	//예약 가능 시간인지 체크
	public static boolean isOverTime(String dateStr, int overTime){
		try {
			/*int strLen = dateStr.length();
			if (strLen != DEFAUT_DATE_TIME_FORMAT_LEN) {
				dateStr = getFormatDateTime(dateStr+"00");
